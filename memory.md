...
These are source-level follow-ups. Backend findings remain open unless noted below.
...
2. **Resolved: rapid route navigation displayed stale content.** The router now guards stale responses and errors with a navigation sequence. The regression script exercises delayed responses and fast navigation.
3. **Resolved: route matching accepted arbitrary leading paths.** Complete normalized paths are now matched; nested aliases and prototype names resolve to the not-found page. Regression checks cover these cases.
...
- The GitHub Pages API confirmed `main` branch/root publishing on 2026-09-20. `_headers` and `_redirects` are not active there; the observed live response still lacks the intended security headers.
...
## Public-site release review — 2026-09-20

- The user authorized final website fixes and a Git push. Started from clean `main`
  at `8ed56212`, then fast-forwarded to `aadebb7a` to preserve intervening automated
  Trading data commits. This pass changes only public-site code, checks, and handoffs.
- Retained the studio layout, prominent particles, immediate cursor, warm light
  theme, and native English/Bulgarian selector. All 20 service CTAs now use native
  links; 19 preserve service context on contact and one opens the existing tool suite.
  Showcase actions remain independent. No new dependency or asset was added.
- Added `aria-current` navigation, heading focus after SPA navigation, immediate
  initial home display, a local skip-link action that preserves contact drafts,
  and a visible recovery link for a prepared email draft after a failed submission.
- Asset version `20260920a`; all ten route shells regenerated. Node regression,
  syntax, CSP hash, and whitespace checks pass. Browser review covers 88 combinations
  of 11 routes, two languages, and four theme/viewport pairs. No horizontal overflow,
  duplicate visible H1s, or broken visible images were found. The six showcase URLs
  returned HTTP 200. Contact requests were mocked; no live brief was sent.
- Pages API confirmed legacy publication from `main` at `/`. Publication must be
  verified against the exact commit's Pages workflow and matching live asset bytes.
  This note records the pre-push validation snapshot, not a deployment receipt.
