...
export function createTimeToUI(makeStorage) {
  ...
  $('timer-repeat').onchange=dateState;$('timer-search').oninput=render;$('timer-filter').onchange=render;$('timer-refresh').onclick=refresh;
  ...
  function stop(){epoch++;repo=null;entries=[];reset();$('timer-editor').open=false;$('timer-filter').value='all';$('timer-search').value='';lock(false);render();status('')}
