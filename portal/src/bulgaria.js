...
import {publicPhoto,placeMedia,photoCredit} from './explorer-photo.js';
...
export function createBulgariaUI(makeStorage,makePhotos){
 ...
 function discoveryCard(p,index,nearby=false){
  const card=el('article','discovery-card'),button=el('button','recommend-place');button.type='button';button.dataset.recommend=p.id;button.onclick=()=>choose(p.id,true);
  const content=el('span','discovery-content');
  content.append(el('span','recommend-kicker',nearby?(index===0?'Closest unvisited':p.category):p.category),el('strong','discovery-name',p.name),el('span','recommend-distance',location?`${formatDistance(distanceKm(location,p))} away · ${p.region}`:p.region),el('span','recommend-action','Explore place ↗'));
  button.append(placeMedia(p,details[p.id]?.photo),content);card.append(button);
  const credit=photoCredit(details[p.id]?.photo);if(credit)card.append(credit);return card;
 }
 function recommendations(visits){
  const show=explorerView==='nearby'&&!$('bg-search').value.trim();$('bg-recommendations').hidden=!show;
  if(!show)return;
  const candidates=filterPlaces(places,{region:$('bg-region').value,category:$('bg-category').value,state:'unvisited',visits}).filter(p=>!photosOnly||publicPhoto(details[p.id]?.photo));
  const illustrated=candidates.filter(p=>publicPhoto(details[p.id]?.photo));
  const preferred=illustrated.filter(p=>['bts-452','bts-476','bts-497'].includes(p.id));
  const picks=location?recommendPlaces(candidates,visits,location):[...preferred,...illustrated.filter(p=>!preferred.includes(p))].slice(0,3);
  $('bg-recommend-title').textContent=!location?'A few places to fall for':picks.length?'Your next discoveries':'No new places match';
  $('bg-recommend-note').textContent=!location?'A little inspiration. Tap Find places near me for your closest unvisited places.':picks.length?'Unvisited places matching your filters, closest first. Distances are straight line.':'Try another category or clear your filters.';
  $('bg-recommend-list').replaceChildren(...picks.map((p,index)=>discoveryCard(p,index,!!location)));
 }
 function assets(){
 ...
 function closeDetail(){
  ...
  const returnButton=[...$('bg-results').querySelectorAll('button')].find(button=>button.dataset.place===returnId);(returnButton||[...$('bg-recommend-list').querySelectorAll('button')].find(button=>button.dataset.recommend===returnId)||[...$('bg-map-picks').querySelectorAll('button')].find(button=>button.dataset.mapDiscovery===returnId)||document.querySelector('button[data-explorer-view][aria-pressed="true"]')).focus({preventScroll:true});
 }
 ...
 function showPhoto(place){
  ...
  const photo=publicPhoto(details[place.id]?.photo),img=$('bg-photo-image');img.removeAttribute('src');$('bg-photo').hidden=true;
  $('bg-photo-missing').hidden=!!photo;$('bg-photo-missing-link').href=googlePlaceUrl(place);
  if(!photo)return;
  $('bg-photo').hidden=false;img.alt=photo.alt||place.name;img.src=photo.url;$('bg-photo-link').href=photo.page;
  img.onerror=()=>{if(photoId===place.id){$('bg-photo').hidden=true;$('bg-photo-missing').hidden=false}};
 ...
 async function locate(){
  if(!navigator.geolocation){$('bg-location-status').textContent='This browser cannot provide your location.';$('bg-location-status').classList.add('error');return}
  $('bg-location-status').classList.remove('error');const request=++locationRequest,gen=epoch;$('bg-locate').disabled=true;$('bg-location-status').textContent='Finding your location…';
  ...
  function locationFailure(error){if(request!==locationRequest||gen!==epoch)return;$('bg-locate').disabled=false;$('bg-location-status').textContent=locationError(error);$('bg-location-status').classList.add('error')}
 }
 function clearLocation(){
  $('bg-location-status').classList.remove('error');locationRequest++;location=null;map?.setLocation(null);$('bg-locate').disabled=false;$('bg-locate').textContent='◎ Find places near me';$('bg-clear-location').hidden=true;
 ...
 function render(){
  ...
  const visible=sortPlaces(filterPlaces(places,{query:$('bg-search').value,region:$('bg-region').value,category:$('bg-category').value,state:explorerView==='visited'?'visited':$('bg-filter').value,visits}).filter(p=>!photosOnly||publicPhoto(details[p.id]?.photo)),$('bg-sort').value,location);
  document.querySelectorAll('[data-bg-category]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.bgCategory===$('bg-category').value)));
  $('bg-only-photos').setAttribute('aria-pressed',String(photosOnly));
  $('bg-clear-filters').textContent='Clear filters';
  $('bg-active-filters').hidden=!($('bg-search').value||$('bg-region').value||$('bg-category').value||$('bg-filter').value!=='all'||photosOnly);
  $('bg-active-filters').textContent=[ $('bg-region').value, $('bg-category').value, photosOnly?'With photos':'', $('bg-filter').value!=='all'?$('bg-filter').selectedOptions[0].textContent:'', $('bg-search').value?`“${$('bg-search').value}”`:'' ].filter(Boolean).join(' · ');
  $('bg-map-list-count').textContent=`Browse ${visible.length} places →`;
  ...
  $('bg-results').replaceChildren(...page.map(p=>{
   const row=el('li','place-card'),button=el('button','place-card-open');button.type='button';button.setAttribute('aria-pressed',String(p.id===selected));button.dataset.place=p.id;
   const content=el('span','place-card-content'),title=el('strong','place-title',p.name),subtitle=el('span','place-subtitle',p.bg);subtitle.lang='bg';
   content.append(el('span','place-card-eyebrow',`${p.category} · No. ${p.number}`),title,subtitle,el('span','place-row-meta',p.region));
   const footer=el('span','place-card-footer');footer.append(el('span',visits.has(p.id)?'place-row-proof':'place-unvisited',visits.has(p.id)?'✓ Visited':'Still to explore'));
   footer.append(el('span','place-row-distance',location?`${formatDistance(distanceKm(location,p))} away`:'View place →'));content.append(footer);
   button.append(placeMedia(p,details[p.id]?.photo),content);button.setAttribute('aria-label',`${p.name}, ${visits.has(p.id)?'visited':'not visited'}`);button.onclick=()=>choose(p.id,true);row.append(button);
   const credit=photoCredit(details[p.id]?.photo);if(credit)row.append(credit);return row;
  }));
  const illustrated=visible.filter(p=>!visits.has(p.id)&&publicPhoto(details[p.id]?.photo)).slice(0,3);
  $('bg-map-discoveries').hidden=!illustrated.length;
  $('bg-map-picks').replaceChildren(...illustrated.map((p,index)=>{const card=discoveryCard(p,index);const button=card.querySelector('button');delete button.dataset.recommend;button.dataset.mapDiscovery=p.id;return card}));
 ...
 async function refresh({force=false}={}){
  ...
  try{
   ...
   if(publicData.status==='fulfilled'){
    const data=publicData.value;details=data.details?.places||{};photoId=null;
    if(!places.length){
 ...
 $('bg-map-list-count').onclick=()=>{setView('nearby');$('bg-browser').scrollIntoView({block:'start'})};
 document.querySelectorAll('[data-bg-category]').forEach(button=>button.onclick=()=>{resultPage=0;$('bg-category').value=button.dataset.bgCategory;render()});
 $('bg-only-photos').onclick=()=>{photosOnly=!photosOnly;resultPage=0;render()};
 $('bg-clear-filters').onclick=()=>{resultPage=0;$('bg-search').value='';$('bg-region').value='';$('bg-category').value='';$('bg-filter').value='all';photosOnly=false;render()};
 ...
 function stop(){
  ...
  photosOnly=false;lock(false);setView('map');status('');
 }
