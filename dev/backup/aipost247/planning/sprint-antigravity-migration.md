...
| ✅ P2-3-scaffold | **DONE (v1.3.0)** — `aipost247/cli_provider.py` adds **Antigravity (`agy -p`)** and **Codex (`codex exec`)** as selectable providers with login dispatch, wired into `generate_text`, the run-loop login gate, terminal `setup`, and the dashboard dropdown + "Вход с акаунт" button. **STILL NEEDS the P0-1 spike** to confirm exact flags/model ids/auth-file paths on a machine that has `agy`/`codex` installed (specs are centralised in `PROVIDERS` for one-line tweaks). | done | `cli_provider.py` | — |
